...
table(subpop$Other.accession.ID %in% met_rr_control$NSFTV_ID)
table(met_rr_control$NSFTV_ID %in% subpop$Other.accession.ID) #all in suspop txt file.
all(met_rr_control$NSFTV_ID == met_rr_stress$NSFTV_ID)
```
...
Met_control_suspop = pca_sub_func(geno = met_rr_control, Gchnt = met_rr_control)
table(Met_control_suspop$Subpopu)

Met_stress_suspop = pca_sub_func(geno = met_rr_stress, Gchnt = met_rr_control)
table(Met_stress_suspop$Subpopu)
```

```{r, echo = TRUE, eval=TRUE}
pca_plot_drawing_func = function(suspop, name){
  
  suspop = suspop %>% group_by(Subpopu) %>% mutate(Subpopulation = paste0(Subpopu, ":", n()))
  
  cc.pca = prcomp(suspop[,-c(1, ncol(suspop), ncol(suspop)-1)], scale. = T)
  
  p = autoplot(cc.pca, data = suspop, colour = "Subpopulation", size = 2.5) + 
    theme_bw()+
...
pca_plot_drawing_func(suspop = G.suspop, name = 'geno')
pca_plot_drawing_func(suspop = Met_control_suspop, name = 'Control')
pca_plot_drawing_func(suspop = Met_stress_suspop, name = 'Stress')
```
